...
def helium(specific_timestamp, results, currently_working, guttedwords, dic, desirable_words, threshold, coords, mandatory_words, cw_width, cw_height):
    ...
    if bool(mandatory_words):
        (gutted_mand, discard) = gut_words(mandatory_words)
...
def get_message():
    ...
    if request.method == "GET":
        return jsonify({"mandatory_words": mandatory_words, "ought_you_continue_get_requests": currently_working, "results": results, "resultcount": len(results)})
        ...
        def begin_crosswordwizard():
            (supergut, superdict, desirable_words) = prepare_helium(fivers, data["banned_words"], data["desirable_words_unfiltered"])
